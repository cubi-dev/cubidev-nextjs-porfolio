...
const SkillsSection = () => {
  ...
  const deleteTechnology = (label: string) => {
    const filteredTechnologies = technologies.filter((tech) => tech.label !== label);
        ...
        <CustomOutlineDiv>
          <div className="grid grid-cols-3 gap-y-4 md:grid-cols-6 md:gap-y-8 lg:grid-cols-8 lg:gap-y-12">
            {technologies.map((technology, index) => (
              <TechDetails {...technology} key={index} editTechnology={editTechnology} deleteTechnology={deleteTechnology}/>
            ))}
          </div>
        </CustomOutlineDiv>
      </div>
